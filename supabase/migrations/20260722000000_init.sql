...
-- ── Signup helpers (called by handle_new_user in section 9). Defined here, before the
-- section 11 REVOKE, so the blanket revoke strips their default PUBLIC execute grant — they
-- are internal, never client-callable RPCs. Pure functions, no table dependency.

-- A stable adjective-noun handle, e.g. 'quiet-otter'. The two lists give 100 x 100 = 10,000
-- combinations, which keeps handle_new_user's retry loop effectively collision-free well past
-- 5,000 accounts per campus (at 5,000 taken, a single draw misses ~50% of the time, so 20
-- retries exhausting is astronomically unlikely). Words are deliberately plain and
-- campus-neutral — no adjective/noun pairing should read as a slur or a real name.
...
declare
  adjectives text[] := array[
    'quiet','brisk','amber','cobalt','dusty','faded','gentle','honest','ivory','jolly',
    'keen','lucky','mellow','nimble','olive','plucky','quaint','rusty','silent','tidy',
    'umber','vivid','wary','zesty','ashen','bold','calm','deft','eager','fair',
    'glad','hazy','idle','jaunty','kindly','lofty','misty','noble','opal','patient',
    'rapid','sober','trusty','upbeat','velvet','witty','young','zippy','arctic','breezy',
    'cozy','dapper','earnest','frosty','golden','humble','inky','jade','lively','marble',
    'nifty','oaken','placid','quirky','rugged','slate','tawny','usual','wistful','airy',
    'blithe','cheery','downy','easy','fleet','grassy','hearty','jovial','lunar','minty',
    'north','plush','ripe','sandy','tender','vernal','warm','amiable','bright','cinder',
    'dawn','ember','fond','grove','hollow','linen','maple','pebble','river','stone'
  ];
  nouns text[] := array[
    'otter','finch','maple','cedar','heron','willow','sparrow','beacon','harbor','meadow',
    'pebble','lantern','compass','falcon','juniper','marten','pigeon','quill','robin','sable',
    'thistle','vireo','walnut','yarrow','acorn','badger','cricket','dahlia','egret','fennel',
    'gopher','hazel','iris','jasper','kestrel','lynx','mallow','nutmeg','oriole','plover',
    'quince','raven','sorrel','tulip','urchin','violet','wren','yucca','aspen','birch',
    'clover','dune','elm','ferry','grebe','hollow','ibis','jetty','kelp','locket',
    'marsh','newt','oak','poppy','quokka','reed','swift','teal','umbra','vale',
    'wharf','xebec','yew','zephyr','anchor','bramble','cove','delta','ember','fjord',
    'glade','harp','inlet','jade','knoll','ledge','moss','nook','orchard','prairie',
    'ridge','summit','tarn','vista','wick','arbor','brook','crag','fern','grove'
  ];
begin
...
-- Deterministic plate background color from the user id. Same id always yields the same
-- color (so a plate is stable for the account's life, like the handle), chosen from a small
-- muted, institutional palette — cool and worn-in per the design direction, and deliberately
-- NOT color.signal (cobalt = commerce) or color.token (amber = rewards), which carry meaning.
create or replace function plate_color_for(p_id uuid)
returns text
language sql
immutable
set search_path = public
as $$
  select (array[
    ...
    '#647084', -- steel
    '#705F52'  -- umber
  ])[1 + (('x' || substr(md5(p_id::text), 1, 8))::bit(32)::bigint % 8)::int];
$$;
...
-- NEITHER read nor delete is gated on is_active_account(), and that pairing is deliberate.
-- A suspended user being force-logged-out must be able to clear their own push token, or
-- their device keeps receiving pushes for an account they can no longer open. DELETE cannot
-- stand alone: Postgres applies the SELECT policy to read the rows a `DELETE ... WHERE`
-- targets, so gating the read would silently make the delete match zero rows — the exact
-- leak, reintroduced. A user seeing only their OWN device tokens is not a cross-user leak,
-- so there is nothing to gate here. (Caught by rls-test case 13.)
...
-- 12. Signup helpers
-- ============================================================
-- generate_handle() and plate_color_for(uuid) are implemented in section 3, BEFORE the
-- section 11 REVOKE, so they are internal and not client-callable. handle_new_user (section
-- 9) calls both. See section 3 for the word lists and the deterministic color derivation.
